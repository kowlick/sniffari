    ...
    for (let y = 1; y < map.height - 1; y++)
      for (let x = 1; x < map.width - 1; x++)
        if (
          ...
          baffles++;
    assert.ok(baffles >= 4, `${label}: only ${baffles} border-adjacent obstacles`);
  });
});
...
test('generated maps stay inside the density bounds from DESIGN.md §4.4', () => {
  forEachGenerated((map, label) => {
    ...
    assert.ok(
      s.walkableFraction > 0.45 && s.walkableFraction < 0.85,
      `${label}: ${(s.walkableFraction * 100).toFixed(0)}% walkable`,
    );
    assert.ok(s.stoppers >= 2, `${label}: ${s.stoppers} stopping points`);
    assert.ok(
